  ...
  return (
    <div className="container mt-4">
          ...
          <div className="card shadow-sm h-100">
            ...
            <div className="card-body" style={{ maxHeight: '400px', overflowY: 'auto' }}>
              {loading ? (
                ...
                <p className="text-muted text-center">No hay actividades recientes</p>
              ) : (
                <ul className="list-group list-group-flush">
                  {actividades.map((act) => (
                    <li key={act.id} className="list-group-item px-0">
                      <small className="text-muted">{formatearFecha(act.fecha)}</small>
                      <p className="mb-0 small">{act.descripcion}</p>
                      <span className="badge bg-secondary small">{act.tipo}</span>
                    </li>
                  ))}
                </ul>
              )}
          ...
          <div className="card shadow-sm h-100">
            ...
            <div className="card-body" style={{ maxHeight: '400px', overflowY: 'auto' }}>
              {loading ? (
                ...
                <p className="text-muted text-center">No hay notificaciones nuevas</p>
              ) : (
                <div className="list-group list-group-flush">
                  {notificaciones.map((notif) => (
                    ...
                    >
                      ...
                      <span className={`badge bg-${getTipoColor(notif.tipo)} small`}>
                        {notif.tipo}
                      </span>
                    </div>
                  ))}
          ...
          <div className="card shadow-sm h-100">
            ...
            <div className="card-body" style={{ maxHeight: '400px', overflowY: 'auto' }}>
              {loading ? (
                ...
                <p className="text-muted text-center">No hay eventos próximos</p>
              ) : (
                <ul className="list-group list-group-flush">
                  {eventos.map((evento) => (
                    <li key={evento.id} className="list-group-item px-0">
                      <h6 className="mb-1">{evento.titulo}</h6>
                      <p className="mb-1 small text-muted">{evento.descripcion}</p>
                      <div className="d-flex gap-2 align-items-center">
                        <span className={`badge bg-${getTipoColor(evento.tipo)}`}>
                          {evento.tipo}
                        </span>
                        <small className="text-muted">{formatearFecha(evento.fecha_inicio)}</small>
                      </div>
                    </li>
                  ))}
                </ul>
      ...
      <div className="alert alert-info mt-4" role="alert">
        ...
        <ul className="mb-0">
          <li>
            <strong>3 parciales por semestre</strong>, cada parcial sobre 14 puntos
          </li>
          <li>
            <strong>Componentes:</strong> Tarea (20%), Informe (20%), Lección (20%), Examen (40%)
          </li>
          <li>
            <strong>Aprobación:</strong> Mínimo 42.10 puntos acumulados (de 42 posibles)
          </li>
          <li>
